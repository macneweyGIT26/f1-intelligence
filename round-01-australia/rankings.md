# F1 STRATEGIC INTELLIGENCE — RANKINGS
## Round 1: Australia 2026 (Structured Data Layer)

**Tabs:** [ [Overview](/viewer.html?file=/round-01-australia/overview.md) ] [ [Rankings](/viewer.html?file=/round-01-australia/rankings.md) ] [ [Engine View](/viewer.html?file=/round-01-australia/engines.md) ] [ [Strategy](/viewer.html?file=/round-01-australia/strategy.md) ] [ [Betting Edge](/viewer.html?file=/round-01-australia/betting-edge.md) ]

---

**Purpose:** Data verification layer. Round 1 — No momentum calculations or trend analysis.

---

## TESTING SNAPSHOT (Pre-Season, Informational Only)

**Source:** FIA Pre-Season Testing (Barcelona, Feb 26–28, 2026)  
**Note:** Testing data NOT used for trend analysis, momentum calculations, or competitiveness ranking. Informational only; teams sandbagging/hiding performance is standard.

| Rank | Driver | Team | Engine | Best Lap (Testing) | Laps Completed | Notes |
|------|--------|------|--------|-------------------|----------------|----|
| TBD | TBD | TBD | TBD | — | — | Awaiting testing results |

---

## PREDICTED GRID (Round 1, Pre-Qualifying)

**Based on:** 2025 driver form + estimated 2026 car competitiveness  
**Confidence:** LOW

| Predicted Pos | Driver | Team | Engine | 2025 Grid Average | 2026 Adjustment | Notes |
|---|---|---|---|---|---|---|
| TBD | TBD | TBD | TBD | — | TBD | Awaiting qualifying results |

---

## ENGINE SUPPLIER RANKING (2026 Season Start)

**Note:** 2025 rankings NOT used as predictors. Each engine supplier starts fresh with 2026 specification.

| Rank | Engine Supplier | Assigned Teams | 2026 Spec Status | Notes |
|------|-----------------|-----------------|-----------------|-------|
| TBD | Mercedes | [Mercedes, McLaren, Aston Martin, Williams, TBD] | TBD | Awaiting testing confirmation |
| TBD | Honda | [Red Bull Racing, Racing Bulls, TBD] | TBD | Awaiting testing confirmation |
| TBD | Ferrari | [Ferrari, Haas, TBD] | TBD | Awaiting testing confirmation |
| TBD | Alpine/Renault | [Alpine, TBD] | TBD | Awaiting testing confirmation |
| TBD | Audi | [Audi, TBD] | TBD | Entry status TBD |
| TBD | Cadillac | [Cadillac (NEW TEAM)] | NO PRIOR DATA | NEW ENTRANT — Confidence VERY LOW; no historical reference |

**2026 Grid (11 Teams):**
Ferrari, Red Bull Racing, Mercedes, McLaren, Aston Martin, Williams, Alpine, Haas, Racing Bulls, Audi, Cadillac

---

**Status:** Round 1 Initialization — Data TBD

---

**Tabs:** [ [Overview](/viewer.html?file=/round-01-australia/overview.md) ] [ [Rankings](/viewer.html?file=/round-01-australia/rankings.md) ] [ [Engine View](/viewer.html?file=/round-01-australia/engines.md) ] [ [Strategy](/viewer.html?file=/round-01-australia/strategy.md) ] [ [Betting Edge](/viewer.html?file=/round-01-australia/betting-edge.md) ]
